    ...
    const attackSvgArt = (() => {
      ...
      register('cast-outer', `${seal(moon,91)}${repeat(8,i=>`<g transform="rotate(${i*45})"><path d="M0-96L6-86L0-76L-6-86Z" fill="#392d63" stroke="#ebd9ff" stroke-width="1.2"/><path d="M0-88L2-85L0-82L-2-85Z" fill="#fff2cf"/></g>`)}<path d="M0-62L53.69 31H-53.69ZM0 62L53.69-31H-53.69Z" fill="none" stroke="#c4a8f7" stroke-width=".8"/>`, 'moon', 384);
      register('cast-inner', `<circle r="94" fill="url(#halo)"/>${runes(58,12,'#c5bcff',15)}<circle r="40" fill="none" stroke="#e9d9ff" stroke-width="1.3"/><circle r="35" fill="none" stroke="#b09adf" stroke-width=".6"/>${repeat(3,i=>`<g transform="rotate(${i*120})"><path d="M0-27C28-22 31 11 8 24C17 5 11-13 0-27Z" fill="url(#metal)"/><path d="M-4-29L-10-37L-8-25" fill="#ddd6ff"/></g>`)}<circle r="6" fill="#fff1dd"/><path d="M0-15V-8M0 8V15M-15 0H-8M8 0H15" stroke="#e1c7ff"/>`, 'moon', 256);
      register('cast-burst', `<circle r="86" fill="url(#halo)"/>${repeat(12,i=>`<g transform="rotate(${i*30})"><path d="M0-95L4-67L0-34L-4-67Z" fill="url(#metal)"/><path d="M-9-50L0-64L9-50" fill="none" stroke="#e9d2ff" stroke-width="1.2"/></g>`)}${runes(76,12,'#bfa7ee',15)}<circle r="27" fill="none" stroke="#f7e9ff" stroke-width="3"/>`, 'moon', 256);
    ...
    function drawSvgCast(ctx, cast, originX = 0, originY = 0) {
      ...
      attackSvgArt.draw(ctx, 'cast-outer', x,y,diameter,diameter,reduced ? 0 : cast.angle*.22,.82);
      attackSvgArt.draw(ctx, 'cast-inner', x,y,diameter,diameter,reduced ? 0 : -cast.angle*.36,.48+remaining*.32);
    }
